...
| D6 | Memorystore Standard-tier instance + VPC egress path attached to Cloud Run + **"CPU always allocated"** | Task #136 (Cloud Run config + Memorystore provisioning) | **Resolve every revision receiving traffic** from `gcloud run services describe … --format=json` (`status.traffic[?percent>0].revisionName`), then for EACH such revision run `gcloud run revisions describe <rev>` and confirm on that revision's own manifest: a VPC egress path is attached (connector annotation OR a valid `network-interfaces` annotation with non-empty subnetwork per §4c step 2) AND `metadata.annotations."run.googleapis.com/cpu-throttling"="false"` (PR #31 §3 W2 — always-allocated CPU is required for the 30 s probe task to fire reliably). Reading only `spec.template` on the service description is REFUSED — the template may differ from the revision serving traffic (canary at 0 %, stalled rollout, etc.), which is the same defect PR #36 review round-2 flagged in `analyze_cloud_run`'s VPC parser. |
