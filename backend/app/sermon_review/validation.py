...
def validate_workbook(
    ...
    *,
    ...
    missing_cols = _missing_columns(rows, headers)
    if missing_cols:
        report.rows.append(
            ValidationRow(
                row=0,
    ...
    for idx, row in enumerate(rows, start=2):  # data starts at workbook row 2
        ...
        if app_cell != segment.appTranslation:
            _emit(
                idx,
                segment_id,
                "warn",
        ...
        if not reviewed_cell.strip():
            _emit(
                ...
                "warn",
                "EMPTY_REVIEW",
                "Reviewed Translation is empty — falling back to "
...
def _missing_columns(
    rows: list[dict[str, str]], headers: Iterable[str] | None
) -> list[str]:
